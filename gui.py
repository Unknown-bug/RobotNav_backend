...
class GUI(tk.Tk):
    ...
    def draw_grid(self):
        for i in range(self.grid.rows): # x0, y0, x1, y1 is positional data of a polygon
            for j in range(self.grid.cols):
                ...
                if self.check_goal(i, j): # Goal cell(s) as lime color
                    self.canvas.create_rectangle(x0, y0, x1, y1, fill="lime", tags="goal")
                elif (i, j) == self.start: # Match starting cell as red color
                    self.canvas.create_rectangle(x0, y0, x1, y1, fill="red", tags="start")
    ...
    def display_next_step(self):
        self.canvas.tag_raise("trace")
        self.canvas.tag_raise("goal") 
        # self.canvas.tag_raise("path")
        self.canvas.tag_raise("start")
        if self.current_traversed < (len(self.traversed)):  # If traversed reach n-1 step
            ...
            self.after(20, self.display_next_step)  # Update traversed expansion every 0.2 seconds

        elif self.current_traversed == (len(self.traversed)): # Once the node expansion reach the goal node, start displaying path
            if self.current_step < (len(self.path)-1): # If path reach n-1 step
                node = self.path[self.current_step]
                x0, y0 = node.x * self.cell_size, node.y * self.cell_size
                x1, y1 = x0 + self.cell_size, y0 + self.cell_size
                self.canvas.create_rectangle(x0, y0, x1, y1, fill="yellow", tags="trace") # Path displayed as yellow color.
                self.current_step += 1 # Add step variable  by 1 after each function call
                self.canvas.create_text(x0 + 13, y0 + 10, text=f"{self.current_step - 1}", anchor="nw", fill="black", tags="path")
